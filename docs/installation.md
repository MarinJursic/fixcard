...
paste with a per-invocation completion token. Its raw, hidden-input reader uses
the same flow on Unix and Windows, and the input is used once and not saved.
